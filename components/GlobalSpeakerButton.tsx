...
export function GlobalSpeakerButton() {
  ...
  const text = getCurrentPageText(pathname);

  return <SpeakerButton text={text} />;
}

function getCurrentPageText(pathname: string | null) {
  if (!pathname || pathname === "/") return getHomeAudioText();
  ...
  const topScams = scams.slice(0, 3).map((scam) => scam.title).join("，");
